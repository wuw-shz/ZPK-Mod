...
import { DatabaseZPK, settingUI, zpkModOn } from "@zpk";
...
system.runInterval(() => {
  ...
  for (const player of world.getAllPlayers()) {
    ...
    if (
      (getMovement(player).has("Forward") ||
        getMovement(player).has("Backward") ||
        getMovement(player).has("Left") ||
        getMovement(player).has("Right")) &&
      !db.befWalk
    ) {
      db.befWalk = true;
    ...
    const gui: GUI = {
      main: {
        labels: [
          `§${db.tc1}X §${db.tc2}${pos.x.toFixed(db.pTF)} §${db.tc1}Y §${db.tc2}${pos.y.toFixed(db.pTF)} §${db.tc1}Z §${
            db.tc2
          }${pos.z.toFixed(db.pTF)}`,
          `${db.showpit ? `§${db.tc1}P §${db.tc2}${rot.x.toFixed(db.rTF)}` : ""}${db.showpit && db.showfac ? " " : ""}${
            db.showfac ? `§${db.tc1}F §${db.tc2}${rot.y.toFixed(db.rTF)}` : ""
          }`,
        ],
      ...
      utils: {
        labels: [
          ...
          `§${db.tc1}Last landing §8[§${db.tc2}${db.landx.toFixed(db.pTF)}§8, §${db.tc2}${db.landy.toFixed(
            db.pTF
          )}§8, §${db.tc2}${db.landz.toFixed(db.pTF)}§8]`,
          `§${db.tc1}Hit §8[§${db.tc2}${db.hitx.toFixed(db.pTF)}§8, §${db.tc2}${db.hity.toFixed(db.pTF)}§${
            db.tc1
          }§8, §${db.tc2}${db.hitz.toFixed(db.pTF)}§8]`,
          `§${db.tc1}Offset §${db.tc2}${NA(db.os)} §${db.tc1}(X, Z) §8[§${db.tc2}${NA(db.osx)}§${db.tc1}§8, §${
            db.tc2
          }${NA(db.osz)}§8]`,
          `§${db.tc1}PB §${db.tc2}${NA(db.pb)} §${db.tc1}(X, Z) §8[§${db.tc2}${NA(db.pbx)}§8, §${db.tc2}${NA(
            db.pbz
          )}§8]`,
        ...
        conditions: [
          ...
          db.separateGui
            ...
            : db.showpos || db.showpit || db.showfac || db.showja || db.showhita,
          db.showspeed,
          db.showttspeed,
          db.showtier,
          db.showland,
    ...
    if (db.separateGui) {
      switch (db.idx) {
        case 1:
          db.idx = 2;
          player.onScreenDisplay.setTitle(
            "!&§r§f" + gui.main.labels.filter((l, i) => gui.main.conditions[i]).join("\n")
          );
          break;
        case 2:
          db.idx = 1;
