...
import { Vector, print, startTime, sleep } from "@lib/minecraft";
import { DatabaseZPK, InitialDataType, settingUI, zpkModOn } from "@zpk";
...
function updateMovementState(player: Player, db: InitialDataType, fullVel: number) {
  ...
  if (
    currentMovement.has("Forward") ||
    currentMovement.has("Backward") ||
    currentMovement.has("Left") ||
    currentMovement.has("Right")
  ) {
    if (!db.befWalk) {
      db.befWalk = true;
...
function updateTiming(player: Player, db: InitialDataType, fullVel: number) {
  ...
  if (!db.HH && db.befJump && db.befWalk && db.jumpTick <= db.walkTick) {
    ...
    if (db.lastTimingTick > 0 && !db.pessi && !db.jam) {
      db.pessi = true;
      db.lastTiming =
        db.lastTimingTick > 0 && db.lastTimingTick <= 100
    ...
    if (TimeDiff(db.walkTick) > 0) {
      db.lastTiming =
        (isBackward ? "BW" : "") + `HH ${msToTicks(db.lastTimingTick).toFixed(2)} Ticks [${db.lastTimingTick}ms]`;
    }
  }
...
function updateOffset(player: Player, db: InitialDataType) {
  ...
  if (
    ...
    vel.y <= 0 &&
    pos.y - vel.y > db.lb.y &&
    -Math.abs(pos.x - db.lb.x - 0.5) + 0.8 >= -1 &&
    -Math.abs(pos.z - db.lb.z - 0.5) + 0.8 >= -1 &&
    !db.befLandLB &&
    db.lbon
  ) {
    db.befLandLB = true;
...
function updateGuiDisplay(player: Player, db: InitialDataType) {
  ...
  const NA = (value: string | number) =>
    typeof value === "number" ? (isFinite(value) ? value.toFixed(db.pTF) : "N/A") : value;
  ...
  const gui: GUI = {
    main: {
      labels: [
        `§${db.tc1}Pos §${db.tc2}${pos.x.toFixed(db.pTF)} §8/ §${db.tc2}${pos.y.toFixed(db.pTF)} §8/ §${
          db.tc2
        }${pos.z.toFixed(db.pTF)}`,
        `${
          db.showrot
            ? `§${db.tc1}Rot (y/p) §${db.tc2}${rot.y.toFixed(db.rTF)} §8/ §${db.tc2}${rot.x.toFixed(db.rTF)}`
            : ""
        }`,
      ],
      conditions: [db.showpos, db.showrot],
    },
    utils: {
      labels: [
        ...
        `\n`,
        `§${db.tc1}Speed (b/t) §8[§${db.tc2}${vel.x.toFixed(db.pTF)}§8, §${db.tc2}${vel.y.toFixed(db.pTF)}§8, §${
          db.tc2
        }${vel.z.toFixed(db.pTF)}§8]`,
        `§${db.tc1}Total Speed (X&Z) §${db.tc2}${fullVel.toFixed(db.pTF)}`,
        `§${db.tc1}Tier §${db.tc2}${db.tier}`,
        `§${db.tc1}Last landing §8[§${db.tc2}${db.landx.toFixed(db.pTF)}§8, §${db.tc2}${db.landy.toFixed(db.pTF)}§8, §${
          db.tc2
        }${db.landz.toFixed(db.pTF)}§8]`,
        `§${db.tc1}Hit §8[§${db.tc2}${db.hitx.toFixed(db.pTF)}§8, §${db.tc2}${db.hity.toFixed(db.pTF)}§8, §${
          db.tc2
        }${db.hitz.toFixed(db.pTF)}§8]`,
        `§${db.tc1}Offset §${db.tc2}${NA(db.os)} §${db.tc1}(X, Z) §8[§${db.tc2}${NA(db.osx)}§${db.tc1}§8, §${
          db.tc2
        }${NA(db.osz)}§8]`,
      ...
      conditions: [
        db.showja,
        db.showhita,
        db.separateGui ? db.showja || db.showhita : db.showpos || db.showrot || db.showja || db.showhita,
        db.showspeed,
        db.showttspeed,
        db.showtier,
        db.showland,
  ...
  if (db.separateGui) {
    player.onScreenDisplay.setTitle(
      (db.shadowLabels ? "&!!&" : "&!&!") +
        "§r§f" +
        gui.utils.labels.filter((l, i) => gui.utils.conditions[i]).join("\n")
    );
    // switch (db.idx) {
    //   case 1:
    //     db.idx = 2;
    ...
    //     db.idx = 1;
    //     player.onScreenDisplay.setTitle(
    //       "&!§r§f" + gui.utils.labels.filter((l, i) => gui.utils.conditions[i]).join("\n")
    //     );
    //     break;
    // }
  } else {
