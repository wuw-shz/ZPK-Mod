...
registerCommand(regInfo, (player, msg, args) => {
    ...
    world.sendMessage(args.get("target") + args.get("pos2") + args.get("pos3"));
    if (args.get("target") === "") {
    ...
    } else if (["target", "tg"].includes(args.get("target") as string) && !args) {
    ...
    } else if (CommandPosition.parseArgs(positionFormat(args.get("target") + args.get("pos2") + args.get("pos3")), 0).result.relativeTo(player, true) !== null) {
