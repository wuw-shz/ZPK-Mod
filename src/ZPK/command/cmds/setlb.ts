...
import { registerCommand, DatabaseZPK, cmdPrefix } from "@zpk";
...
registerCommand(regInfo, (player, msg, args) => {
  ...
  if (args.get("target") === "") {
    ...
    const pos = CommandPosition.parseArgs(
      positionFormat(args.get("target") + args.get("pos2") + args.get("pos3")),
      0
    ).result.relativeTo(player, true);
    db.lbon = true;
    db.lb = vec3(Math.floor(pos.x), pos.y, Math.floor(pos.z));
    Server.command.callCommand(player, "clearpb", ["-s"]);
    ...
    print(
      `§l§c✘ Invalid setlb command input!\nUsage:\n ${cmdPrefix}setlb <targetBlock: "target" or "tg">\n ${cmdPrefix}setlb <position: x y z>`,
      player
    );
  }
});
