...
class StructureManager {
  ...
  save(
    ...
    const saveOptions: StructureCreateOptions = {
  ...
  async saveWhileLoadingChunks(
    name: string,
    start: Vector3,
    end: Vector3,
    dim: Dimension,
    options: StructureSaveOptions = {},
    loadArea: (min: Vector3, max: Vector3) => boolean
  ) {
    const min = Vector.min(start, end);
    const max = Vector.max(start, end);
    const size = Vector.from(regionSize(start, end));
  ...
  load(
    ...
    if (
      struct?.subRegions ||
      this.beyondMaxSize(options.importedSize ?? Vector.ZERO)
    ) {
      ...
      const subStructs = options.importedSize
        ? this.getSubStructs(
            location,
            Vector.add(location, options.importedSize).floor()
          )
        : struct.subRegions;
      for (const sub of subStructs) {
        const subBounds = regionTransformedBounds(
          sub.start.floor(),
          sub.end.floor(),
          Vector.ZERO,
          rotation,
        ...
        try {
          world.structureManager.place(
            name + sub.name,
            dim,
            Vector.sub(subBounds[0], bounds[0]).add(loadPos),
            loadOptions
          );
        } catch {
          error = true;
  ...
  private beyondMaxSize(size: Vector3) {
    return (
      size.x > this.MAX_SIZE.x ||
      size.y > this.MAX_SIZE.y ||
      size.z > this.MAX_SIZE.z
    );
  }
      ...
      for (let y = 0; y < size.y; y += this.MAX_SIZE.y)
        for (let x = 0; x < size.x; x += this.MAX_SIZE.x) {
          ...
          const subEnd = Vector.min(
            subStart.add(this.MAX_SIZE).sub(1),
            size.add([-1, -1, -1])
          );
          const subName = `_${x / this.MAX_SIZE.x}_${y / this.MAX_SIZE.y}_${
            z / this.MAX_SIZE.z
          }`;
