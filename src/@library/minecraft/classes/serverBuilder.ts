...
import { runCommandReturn } from "../@types/classes/ServerBuilder.js";
...
export class ServerBuilder extends EventEmitter {
     ...
     */
    runCommand(command: string, target?: Dimension | Player | Entity | Block): runCommandReturn {
        try {
            const successCount = (target instanceof Block ? world.getDimension("overworld") : target).runCommand(command).successCount;
     ...
     */
    queueCommand(command: string, target?: Dimension | Player | Entity | Block): Promise<runCommandReturn> {
        try {
            if (this.flushingCommands) {
                throw "queue";
            }
            const promise = (target instanceof Block ? world.getDimension("overworld") : target)
