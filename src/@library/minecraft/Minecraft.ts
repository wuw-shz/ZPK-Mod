...
export {
  commandSyntaxError,
  registerInformation as CommandInfo,
} from './@types/classes/CommandBuilder.js';
export {
  StructureSaveOptions,
  StructureLoadOptions,
} from './classes/structureBuilder.js';
...
class ServerBuild extends ServerBuilder {
  ...
  private _buildEvent() {
    ...
    beforeEvents.chatSend.subscribe(data => {
      ...
      if (this.command.getRegistration(command)) {
        data.cancel = true;

        this.command.callCommand(
          data.sender,
          command,
          msg.substring(msg.indexOf(command) + command.length).trim()
        );
      }
    });
    /**
     ...
     */
    beforeEvents.explosion.subscribe(data =>
      this.emit('beforeExplosion', data)
    );
    /**
     ...
     */
    afterEvents.pistonActivate.subscribe(data =>
      this.emit('pistonActivate', data)
    );
    /**
    ...
    beforeEvents.itemUseOn.subscribe(data =>
      this.emit('itemUseOnBefore', data)
    );
     ...
     */
    afterEvents.weatherChange.subscribe(data =>
     ...
     */
    beforeEvents.playerBreakBlock.subscribe(data =>
      this.emit('blockBreak', data)
    );
    /**
     ...
     */
    afterEvents.worldInitialize.subscribe(data =>
      this.emit('worldInitialize', data)
    );
    /**
     * Emit to 'playerChangeDimension' event listener
     */
    afterEvents.playerDimensionChange.subscribe(data =>
