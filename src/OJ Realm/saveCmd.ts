...
// import { DatabaseOJR, Mode, text } from "@oj-realm";
...
//   const db = new DatabaseOJR(sender);
...
//       if (db.get(Mode.practiceData).toggle) return sender.sendMessage(text.saveCommand.notification.inPracticeMode);
...
//       sender.sendMessage(
//         text.saveCommand.notification.save(Math.floor(loc.x), Math.floor(loc.y), Math.floor(loc.z), newName)
//       );
//       server.system.run(() => {
