...
parser = argparse.ArgumentParser(
    description="Syncs the project folder's data with Minecraft (Windows 10/11 only).\nNote: Will only sync CHANGED files in watch mode."
)
parser.add_argument(
    "--watch", "-w", action="store_true", help="Whether to watch for file changes."
)
parser.add_argument(
    "--init",
    choices=["False", "True"],
    default="True",
    help="Whether to initially sync com.mojang before watching file changes.",
)
parser.add_argument(
    "--dest",
    choices=["stable", "preview", "server"],
    default="stable",
    help="The place to sync the addon to",
)
args = parser.parse_args()

if args.dest == "stable":
    com_mojang = os.path.expandvars(
...
behaviour_pack = os.path.join(
    com_mojang, "development_behavior_packs", f"{pack_folder} BP"
)
resource_pack = os.path.join(
    com_mojang, "development_resource_packs", f"{pack_folder} RP"
)
